...
for p in data['people']:
 ...
 body=f'<article class="article static-fallback"><p class="eyebrow">{e(p["category"])} · sourced profile</p><div class="arabic" lang="ar" dir="rtl">{e(p["arabic"])}</div><h1>{e(p["name"])}</h1><p class="lede">{e(p["role"])}</p><p>{e(p["date"])} · {e(p["place"])}</p>{sections}<p class="note">{e(p["uncertainty"])}</p>{passage_html}{sunnah_html}{check_html}{source_html}</article>'
