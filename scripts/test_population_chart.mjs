...
import {chartX,hasPlottableYear,populationChart} from '../public/population-chart.mjs';
...
test('profile charts require a real year inside the supported chart range',()=>{
 assert.equal(hasPlottableYear(null),false);
 assert.equal(hasPlottableYear(undefined),false);
 assert.equal(hasPlottableYear('1946'),false);
 assert.equal(hasPlottableYear(780),false);
 assert.equal(hasPlottableYear(1946),true);
 assert.equal(hasPlottableYear(2026),true);
 assert.equal(hasPlottableYear(2027),false);
});
