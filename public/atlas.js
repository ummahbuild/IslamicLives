import {populationLabel,regionalYear,markersFor} from './atlas-model.mjs';
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
export function mountAtlas(main,chapter='tradition'){
 const abort=new AbortController();let disposed=false,globe=null,timer=null,index=0,data=null;
 ...
 function setChapter(next,manual=true){
  ...
  globe?.update(markers,e.links,data.places);
 }
 async function setup(){
  try{
   ...
   try{
    ...
    const current=data.events[index];globe.update(markersFor(data,current),current.links,data.places);
    let longitude=35;
