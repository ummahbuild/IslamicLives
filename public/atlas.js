...
export function mountAtlas(main,chapter='tradition'){
 ...
 function showMarker(marker){
  ...
  const status=main.querySelector('#atlas-status');if(status)status.textContent=`Focused ${marker.name}.`;
  globe?.focus(marker.lat,marker.lon);
 }
 function setChapter(next,manual=true){
  if(manual)pause();const requested=Number(next);if(Number.isInteger(requested))index=Math.max(0,Math.min(data.events.length-1,requested));
  const e=data.events[index],markers=markersFor(data,e),year=regionalYear(e);
  history.replaceState(null,'',`#spread/${e.id}`);
  document.title=`${e.date} · ${e.title} — Islamic Lives`;
  ...
  main.querySelector('.population-panel').hidden=!e.population;
  ...
  const status=main.querySelector('#atlas-status');if(status)status.textContent=`Chapter ${index+1} of ${data.events.length}: ${e.date}, ${e.title}.`;
 ...
 async function setup(){
  try{
   ...
   try{
    ...
    const current=data.events[index];globe.update(markersFor(data,current),current.links,data.places,cumulativeAtlasContext(data,index));
    let longitude=35;
